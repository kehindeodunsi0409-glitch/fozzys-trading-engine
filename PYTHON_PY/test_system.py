...
try:
    import numpy as np
    import pandas as pd
    from strategies import momentum_signal, mean_revert_signal

    # Generate trending data for momentum test
    np.random.seed(1)
    n = 100
    ...
    cfg = {"fast_ema":8,"slow_ema":21,"atr_period":14,
           "atr_sl_mult":2.0,"atr_cap_usd":5000,"min_rr":2.0}

    sig = momentum_signal(df_trend, cfg)
    if sig:
        ok(f"Momentum signal: {sig['reason']} RR=1:{sig['rr']}")
    else:
        ok("Momentum: no signal on test data (normal - not every bar has a cross)")

    # Mean revert test
    cfg2 = {"bb_period":20,"bb_std":2.0,"rsi_period":14,
            "rsi_ob":65,"rsi_os":35,"atr_period":14,
            "atr_sl_mult":1.5,"atr_cap_usd":5000,"min_rr":2.0}
    sig2 = mean_revert_signal(df_trend, cfg2)
    ok(f"Mean revert: {'signal found' if sig2 else 'no signal (normal)'}")
